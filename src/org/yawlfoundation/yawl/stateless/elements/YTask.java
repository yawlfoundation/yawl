...
import org.yawlfoundation.yawl.stateless.elements.predicate.PredicateEvaluatorCache;
...
public abstract class YTask extends YExternalNetElement {
    ...
    protected final Map<String, String> _dataMappingsForTaskStarting = new HashMap<>();       //[key=ParamName, value=query]
    private final Map<String, String> _dataMappingsForTaskCompletion = new HashMap<>();       //[key=query, value=NetVarName]
    protected YDecomposition _decompositionPrototype;

    // input data storage
    private Element _executingData ;
    private Iterator<Element> _multiInstanceSpecificParamsIterator;
    ...
    public Set<YExternalNetElement> getRemoveSet() {
        return _removeSet != null ? new HashSet<>(_removeSet) : null;
    }
    ...
    public synchronized List<YIdentifier> t_fire()
    ...
    public synchronized YIdentifier t_add(YIdentifier siblingWithPermission, Element newInstanceData)
    ...
    public synchronized boolean t_complete(YIdentifier childID,
    ...
    public synchronized void t_start(YNetRunner runner, YIdentifier child)
            throws YDataStateException, YQueryException, YStateException {
        if (t_isBusy()) {
            startOne(runner, child);
        }
    }
    ...
    private synchronized void t_exit()
            throws YDataStateException, YStateException, YQueryException {
        if (!t_isExitEnabled()) {
    ...
    private void doXORSplit(YIdentifier tokenToSend) throws YQueryException {
    ...
    private void doOrSplit(YIdentifier tokenToSend) throws YQueryException {
    ...
    private void doAndSplit(YIdentifier tokenToSend) {
        if (tokenToSend != null) {
            for (YExternalNetElement element : getPostsetElements()) {
                ((YCondition) element).add(tokenToSend);
            }
    ...
    private boolean evaluateTimerPredicate(String predicate, YIdentifier token) throws YQueryException {
        if (getNetRunner() != null) {
            return getNetRunner().evaluateTimerPredicate(predicate);
    ...
    protected abstract void startOne(YNetRunner parentRunner, YIdentifier id)
            throws YDataStateException, YQueryException, YStateException;
    ...
    public void prepareDataForInstanceStarting(YIdentifier childInstanceID)
        ...
        if (null != getDecompositionPrototype()) {
            _executingData = getStartingDataSnapshot();
        }
    ...
    public synchronized void rollbackFired(YIdentifier childID) {
    ...
    public List<YInternalCondition> getAllInternalConditions() {
        List<YInternalCondition> icList = new ArrayList<>();
    ...
    public String toXML() {
        ...
        List<YExternalNetElement> removeList = new ArrayList<>(_removeSet);
        ...
        for (YExternalNetElement netElement : removeList) {
            ...
            if (netElement instanceof YCondition) {
                ...
                if (maybeImplicit.isImplicit()) {
                    ...
                    YExternalNetElement post = maybeImplicit.getPostsetElements().iterator().next();
