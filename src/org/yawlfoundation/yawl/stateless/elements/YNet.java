...
public final class YNet extends YDecomposition {
    ...
    public Map<String, YExternalNetElement> getNetElements() {
        return new ConcurrentHashMap<>(_netElements);
    }
    ...
    public void initialise() {
        ...
        for (YVariable variable : _localVariables.values()) {
            ...
            if (variable.getInitialValue() != null) {
                addData(new XNode(varElementName,
                        variable.getInitialValue()).toElement());
            } 
            else {
                addData(new Element(varElementName));
            }
        }
    }


    public void setIncomingData(Element incomingData) throws YDataStateException {
        ...
        if (gateway != null) {
            try {
                return gateway.populateCaseData(getSpecification().getSpecificationID(),
                        caseID,
                        new ArrayList<>(getInputParameters().values()),
                        new ArrayList<>(getLocalVariables().values()),
                        _data.getRootElement());
            }
            catch (Throwable t) {
