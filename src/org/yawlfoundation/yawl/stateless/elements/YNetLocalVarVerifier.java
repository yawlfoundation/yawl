...
public class YNetLocalVarVerifier {
    ...
    private final Map<String, LocalTaskMap> _uninitialisedLocalVars;
    ...
    public void verify(YVerificationHandler handler) {
        ...
        _log.debug("Net: {} | Duration: {}", _net.getID(),
    ...
    private void verify(LocalTaskMap map) {
        ...
        for (YTask task : map.getInputTasks()) {
            verify(map, task, task, new ArrayList<>(), new Stack<>());
        }
    ...
    private void verify(LocalTaskMap map, YTask subjectTask,
        ...
        for (YExternalNetElement preElement : baseElement.getPresetElements()) {
            ...
            _log.debug("Net: {} | Local var: {} | Subject: {} | PreElement: {}",
                    _net.getID(), map.getLocalVar().getPreferredName(),
                    subjectTask.getName(), preElement.toString());
    ...
    private class LocalTaskMap {
        ...
        private final YVariable _localVar;
        private final Set<YTask> _inputTasks;
        private final Map<YTask, Boolean> _outputTasks;
