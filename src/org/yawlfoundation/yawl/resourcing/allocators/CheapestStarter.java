...
public class CheapestStarter extends AbstractAllocator {
    ...
    public CheapestStarter() {
        ...
        setDescription("The Cheapest Starter allocator chooses the participant from " +
                       "the distribution set who, on average, has recorded the " +
                       "cheapest cost from item allocation to starting the item.");
    }
        ...
        if (costMap != null) {
            ...
            for (String pid : costMap.keySet()) {
                if (durationMap.containsKey(pid)) {
                    double cost = getCost(costMap.get(pid), "allocated") * durationMap.get(pid);
