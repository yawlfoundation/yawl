...
public class FastestToStart extends AbstractAllocator {

    public FastestToStart() {
        super() ;
        setName(this.getClass().getSimpleName()) ;
        setDisplayName("Fastest to Start") ;
        setDescription("The Fastest to Start allocator chooses the participant from " +
                       "the distribution set who, on average, has recorded the " +
                       "fastest time from item allocation to starting the item.");
    }
    ...
    private String getFastestAverage(Collection<String> pSet, WorkItemRecord wir) {
        ...
        if (pSet != null) {
            Map<String, Long> durationMap = getAvgDurations(
                    EventLogger.event.allocate, EventLogger.event.start, wir);
