...
public class FrmBlockExceptionChoice extends DesignInternalFrame {
	...
	static final String TITLE = "Exception Interaction Point";

	  private static FrmBlockExceptionChoice single = null;
	  private static BlockExceptionChoiceCoordinator blockExceptionChoiceCoordinator;
	  ...
	  private FrmBlockExceptionChoice(BlockExceptionChoiceCoordinator aChoiceCoordinator) {
		    super(TITLE);
		    this.blockExceptionChoiceCoordinator = aChoiceCoordinator;
		    try {
		    	jbInit();
		    }
		    catch (Exception ex) {
		  ...
		  public static FrmBlockExceptionChoice singleton(BlockExceptionChoiceCoordinator
		                                          coord) {
		    //if (single == null) {
		      single = new FrmBlockExceptionChoice(coord);
		    //}
		    return single;
		  }
		  ...
		  private void initializeFields() {
			  List<List> exc = BlockPI.getExceptions();
			  for (List l : exc) {
				  String classID = (String) l.get(1);
				  String procletID = (String) l.get(2);
				  String blockID = (String) l.get(3);
				  String result = classID + "," + procletID + "," + blockID;
		   ...
		   private void initComponents() {
		        ...
		        ignoreButton.setText("Grant More Time");
		    ...
		    private void handleButtonActionPerformed(java.awt.event.ActionEvent evt) {
		    	...
		    	for (List l : exc) {
		    		...
		    		String classIDl = (String) l.get(1);
		    		String procletIDl = (String) l.get(2);
		    		String blockIDl = (String) l.get(3);
		    		if (classID.equals(classIDl) && procletID.equals(procletIDl) && 
		    				blockID.equals(blockIDl)) {
		    			// remove from list
		    			exceptionComboBox.removeItem(item);
		    			// remove from db
		    			BlockPI.deleteException(classID, procletID, blockID);
		    			BlockPI.deleteExceptionCaseSelected(classID, procletID, blockID);
		    			// put it in db for emids
		    			BlockPI.publishExceptionCase(classID, procletID, blockID);
		    			break;
		    		}
		    	}
		    	this.handleButton.setEnabled(false);
		    ...
		    private void ignoreButtonActionPerformed(java.awt.event.ActionEvent evt) {
		    	...
		    	for (List l : exc) {
		    		...
		    		if (classID.equals(classIDl) && procletID.equals(procletIDl) && 
		    			...
		    			BlockPI.deleteExceptionCaseSelected(classID, procletID, blockID);
		    			// put it in db for emids
		    			BlockPI.publishExceptionCase(classID, procletID, blockID+"TIME");
		    			break;
		    		}
		    	}
		    	this.handleButton.setEnabled(false);
