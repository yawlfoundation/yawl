...
public class SpecificationModel implements Serializable {
  ...
  public void setStartingNet(NetGraphModel startingNet) {
    NetGraphModel oldStartingNet = getStartingNet();
    oldStartingNet.setIsStartingNet(false);
    startingNet.setIsStartingNet(true);
  }
  ...
  public boolean isValidDecompositionLabel(String label) {
    ...
    while(decompositionIterator.hasNext()) {
      ...
      if (decomposition.getLabelAsElementName().equals(XMLUtilities.toValidElementName(label))) {
        return false;
      }
    }
    Iterator netIterator = nets.iterator();
    while(netIterator.hasNext()) {
      NetGraphModel net = (NetGraphModel) netIterator.next();
      if (net.getDecomposition().getLabelAsElementName().equals(XMLUtilities.toValidElementName(label))) {
        return false;
      }
    }
    return true;
