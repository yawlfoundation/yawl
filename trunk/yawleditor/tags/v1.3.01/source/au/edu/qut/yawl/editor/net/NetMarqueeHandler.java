...
public class NetMarqueeHandler extends BasicMarqueeHandler {
    ...
    switch (Palette.getInstance().getSelected()) {
      case Palette.FLOW_RELATION: {
        if (currentPort != null && generatesOutgoingFlows(currentPort)) {
          setStartPoint(getNearestSnapPoint(graph.toScreen(currentPort.getLocation(null))));
  ...
  private Point getNearestSnapPoint(Point point) {
  ...
  public PortView getSourcePortAt(Point point) {
    final Point tmp = graph.fromScreen(point);
  ...
  protected void paintConnector(Color fg, Color bg, Graphics g) {
    ...
    if (startPort != null && getStartPoint() != null && getCurrentPoint() != null)
      g.drawLine(getStartPoint().x, getStartPoint().y, getCurrentPoint().x, getCurrentPoint().y);
  }
  ...
  protected void showPort(PortView thisPort) {
    if (thisPort != null) {
      Rectangle r = thisPort.getBounds();
      r.translate(-PORT_BUFFER/2, 
                  -PORT_BUFFER/2);
      r.setSize(r.width  + (2 * PORT_BUFFER), 
                r.height + (2 * PORT_BUFFER));
  ...
  private boolean acceptsIncommingFlows(PortView portView) {
    ...
    return yawlPort.acceptsIncommingFlows() && 
