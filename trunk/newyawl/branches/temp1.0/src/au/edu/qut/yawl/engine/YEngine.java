...
//                query = pmgr.createQuery("select from au.edu.qut.yawl.logging.YCaseEvent where case_id = '" + id + "'");
//                for (Iterator it = query.iterate(); it.hasNext();) {
...
//            YEventLogger.getInstance().setListofcases(map);
            ...
            for (Iterator it = query.iterate(); it.hasNext();) {
                ...
                if (witem.get_dataString() != null) {
                    StringReader reader = new StringReader(witem.get_dataString());
...
//AJH                yper.storeData(runner);
            ...
            yawllog.logCaseCreated(pmgr, caseID.toString(), username, specID);

            runner.continueIfPossible(pmgr);

            runner.start(pmgr);
            _caseIDToNetRunnerMap.put(caseID, runner);
            _runningCaseIDToSpecIDMap.put(caseID, specID);

            if (_interfaceBClient != null) {
                logger.debug("Asking client to add case " + caseID.toString());
                _interfaceBClient.addCase(specID, caseID.toString());
            }
                ...
                if (workItem != null) {
                    if (workItem.getStatus().equals(YWorkItem.statusExecuting)) {
                        ...
                        synchronized (netRunner) {
                            ...
                            try {
                                data = JDOMConversionTools.cleanAmps(data);
...
//                        workItem.setStatusToComplete(pmgr, force);
   ...
   //added method
    ...
    public Document getCaseData(String id) {
