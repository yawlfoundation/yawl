...
public class FilterFactory {
     ...
     */
    public static ResourceFilter getInstance(String filterName) {
        try {
			ResourceFilter newClass = (ResourceFilter)
     ...
     * @return the instantiated class, or null if there was a problem
     */
    public static ResourceFilter getInstance(String filterName, HashMap params) {
        ResourceFilter newClass = getInstance(filterName);
    ...
    //    String[] classes = new File(".").list(new FilterClassFileFilter());
        ...
        String[] classes = new File("./classes/au/edu/qut/yawl/resourcing/filters/").list(
                           new FilterClassFileFilter());

        for (String aClass : classes) {

            // strip off the file extension
            String sansExtn = aClass.substring(0, aClass.lastIndexOf('.'));
            ResourceFilter temp = getInstance(sansExtn);
    ...
    private static class FilterClassFileFilter implements FilenameFilter {
        ...
        public boolean accept(File dir, String name) {
            if (( new File(dir, name).isDirectory() ) ||        // ignore dirs
                ...
                (name.startsWith("ResourceFilter")))            // and the base classes
                return false;
