...
//            	 due to a bug, YAWL converts untyped parameters into creation parameters
    ...
    public String getRedirectURL(ServletContext context, SpecificationData specData,
    	...
    	String url = new String(context.getInitParameter("YAWLXForms") +
                "/XFormsServlet?form=/forms/" + this.getXFormName(specData) +
    ...
    public String getRedirectURL(ServletContext context, TaskInformation taskInfo,
	    ...
	    String url = new String(context.getInitParameter("YAWLXForms") +
	              "/XFormsServlet?form=/forms/" + this.getXFormName(taskInfo) +
	              "&css=yawl.css&xslt=html4yawl.xsl&JSESSIONID="+jsessionid);
     ...
     * @return
     */
    private String getXFormName(TaskInformation taskInfo){
    ...
    public String getHTMLFormName(TaskInformation taskInfo){
    	String result = new String();
    	try{
	        result = URLEncoder.encode(taskInfo.getTaskID()+".jsp", "UTF-8");
    	}
    	catch(UnsupportedEncodingException e){
    		e.printStackTrace();
    	}
     ...
     * @return
     */
    private String getXFormName(SpecificationData specData){
    	String result = new String();
    	try{
    ...
    public String getHTMLFormName(SpecificationData specData){
    	String result = new String();
    	try{
	        result = (URLEncoder.encode(specData.getID()+".jsp", "UTF-8"));
    	}
    	catch(UnsupportedEncodingException e){
    		e.printStackTrace();
    	}
