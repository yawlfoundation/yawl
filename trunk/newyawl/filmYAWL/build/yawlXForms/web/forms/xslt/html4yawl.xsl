...
<xsl:stylesheet version="1.0"
    ...
    <xsl:param name="action-url" select="'http://192.94.227.138:8080/yawlXForms/XFormsServlet'"/>


    <xsl:param name="form-id" select="'chibaform'"/>
    <xsl:param name="form-name" select="//html:title"/>
    ...
    <xsl:variable name="default-css" select="concat($contextroot,$CSSPath,'yawl.css')"/>
            ...
            <xsl:if test="html:base">
                <base>
                    <xsl:attribute name="href">
                        <xsl:value-of select="html:base/@href"/>
                    </xsl:attribute>
                </base>
            </xsl:if>
    ...
    <xsl:template name="getLinkAndStyle">
        <xsl:for-each select="html:link|html:style">
    ...
    <!-- ### copy elements from the html2 namespace to html (without any namespace) by re-creating the     ### -->
