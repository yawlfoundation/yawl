 ...
 * abilities, and property change listener for all the domain object proxys
 * that are inside the graph.
 * 
 * @author catch23
 * @created October 28, 2002
 */
public class CapselaGraph extends JGraph implements Printable, DropTargetListener, DragGestureListener,
    DragSourceListener, PropertyChangeListener {
  ...
  public CapselaGraph(CapselaGraphModel model, GraphEditor graphEditor, SharedNode sharedNode) {
    ...
    // Pressing the mouse button 2 times in a row will allow users to edit the
    // description
    setEditClickCount(2);
    ...
    this.addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent e) {
        if (e.getClickCount() == 3) {
          ...
          Object cell = CapselaGraph.this.getFirstCellForLocation(x, y);
    ...
    GraphLayoutCache cache = new CapselaGraphLayoutCache(getModel(), new ViewFactory(), this, true);
  ...
  /**
   * CapselaGraph will be the property listener for all domain object
   * proxys it contains. So if a proxy's domain object has changed
   * state and the state affects cell renderer output, it will repaint the
   * graph. Since the renderers do not maintain state, they must fetch the state
   * information from the domain object via the proxy, via the cell.
   * 
...
//      Rectangle2D bounds = this.getGraphLayoutCache().getMapping(((DataProxy) source).getGraphCell(), false).getBounds();
...
//    }
  ...
  public void autoSizeAll() {
    CellView[] views = this.getGraphLayoutCache().getMapping(
        JGraphUtilities.getVertices(this.getModel(), DefaultGraphModel.getAll(this.getModel())));
    try {
      Hashtable table = new Hashtable();
      if (views != null) {
        for (int i = 0; i < views.length; i++) {
          CellView view = views[i];
          JComponent component = (JComponent) view.getRendererComponent(this, false, false, false);
          if (component != null) {
            this.add(component);
            component.validate();
...
//      try {
//        ClientOperation.executeCopyCommand((DataProxy) draggingNode.getproxy(), (DataProxy) newParent.getproxy(), location,
//            _graphEditor);
...
//      }
      ...
      if (newParent != oldParent) {
          throw new RuntimeException("implement the copy operation");
//        try {
//          ClientOperation.executeMoveCommand((DataProxy) draggingNode.getproxy(),(DataProxy)  oldParent.getproxy(),(DataProxy)  newParent
...
//        }
   ...
   * graph.
   * @param draggingNode the node to check
...
//    return (notDroppingFolder && notDroppingIntoInstance);
  ...
  private boolean isDropCopy(SharedNode draggingNode) {
  	...
  	return true;
//    // It's a copy operation (rather than a move operation) if you're dragging:
...
//    SharedNode parent2 = (SharedNode) ((DataProxy) this.getproxy()).getSharedNode().getParent();
//    boolean fromNetworkToAnywhere = draggingNode.isInNetworkFolder();
//    boolean fromAnywhereToNetwork = ((DataProxy) this.getproxy()).getSharedNode().isInNetworkFolder();
//    boolean fromComponentsToAnywhere = draggingNode.isInComponentsFolder();
//    boolean fromFlowToFlow = parent1 != null && parent1.getproxy().isFlow() && parent2 != null && parent2.getproxy().isFlow();
//    return fromNetworkToAnywhere || fromAnywhereToNetwork || fromComponentsToAnywhere || fromFlowToFlow;
  }
...
//      DataConnectionEditor editor = new DataConnectionEditor((DataProxy) _proxy, _graphEditor.getFlowEditor());
//      editor.setVisible(true);
//      editor.expandPaths(new TreePath(sourceproxy.getSharedNode().getPath()), new TreePath(sinkproxy.getSharedNode().getPath()));
//    } else {
...
//    }
  ...
  public void startBasicEditingAtCell(final CapselaCell cell) {
	...
	public void clear() {
		...
		// may not be strictly necessary but makes it a little easier to see in profiler what's left
		_graphEditor = null;
		_SharedNode = null;
		_proxy = null;
		_dragSource = null;
