...
/**
 * This version of <code>Properties</code> extends <code>HashMap</code> instead
 * of <code>Hashtable</code> in order to avoid the overwriting of colliding keys.
 * <p>
...
public class Properties extends HashMap {
	...
	/**
	 * Creates a new <tt>Properties</tt> instance with the given initial
	...
	public Enumeration propertyNames() {
		...
		enumerate( h );
		return new HashKeyEnumeration( h );
	}

	private class HashKeyEnumeration implements Enumeration {
		/**
		 * Creates a new enumeration over a given hash map.
		 *
		 * @param hashMap the hash map.
		 */
		HashKeyEnumeration( HashMap hashMap ) {
			_iterator = hashMap.keySet().iterator();
		}//HashKeyEnumeration
	 ...
	 */
	private synchronized void enumerate( HashMap h ) {
		if( defaults != null ) {
