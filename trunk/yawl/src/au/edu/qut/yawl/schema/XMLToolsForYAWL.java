...
public class XMLToolsForYAWL {
    ...
    public void setPrimarySchema(Element element) throws YSchemaBuildingException, YSyntaxException {
        //first of all check that the schema itself is valid
        try {
            _xsd4YAWLBuilder.setSchema(element);
        } catch (Exception e) {
            YSchemaBuildingException f = new YSchemaBuildingException(e.getMessage());
            f.setStackTrace(e.getStackTrace());
            throw f;
     ...
     * Note that it is a syntax error for desired element names and type names to overlap.
