...
public class CheapestCompleter extends AbstractAllocator {

    public CheapestCompleter() {
        super() ;
        setName(this.getClass().getSimpleName()) ;
        setDisplayName("Cheapest Completer") ;
        setDescription("The Cheapest Completer allocator chooses the participant from " +
                       "the distribution set who, on average, has recorded the " +
                       "cheapest cost from item start to its completion.");
    }
        ...
        if (costMap != null) {
            ...
            for (String pid : costMap.keySet()) {
                if (durationMap.containsKey(pid)) {
                    double cost = getCost(costMap.get(pid), "busy") * durationMap.get(pid);
