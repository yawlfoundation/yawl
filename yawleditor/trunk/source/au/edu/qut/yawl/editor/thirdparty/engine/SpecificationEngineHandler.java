...
public class SpecificationEngineHandler {
  ...
  private YSpecification generateEngineSpecification() {
    ...
    engineSpecification.setBetaVersion(YSpecification._Beta7_1);
  ...
  private void generateEngineMetaData(YSpecification engineSpecification) {
    ...
    //BEGIN: MLF
    if (SpecificationModel.getInstance().isJournalise()) {
        metaData.setJournalise(true);
    }
    //END: MLF
  ...
  private void setConditions(YNet engineNet, NetElementSummary editorNetSummary) {
    ...
    while(conditionIterator.hasNext()) {
      ...
      engineCondition.setDocumentation(XMLUtilities.quoteSpecialCharacters(editorCondition.getLabel())); //MLF: add label as documentation
  ...
  private void generateDecompositionParameters(YDecomposition engineDecomposition, 
    ...
    //BEGIN: MLF merge additional attributes
    for(Enumeration enumer = editorDecomposition.getAttributes().keys(); enumer.hasMoreElements();) {
      ...
      engineDecomposition.setAttribute(key, XMLUtilities.quoteSpecialCharacters(editorDecomposition.getAttribute(key)));
    }
    //END: MLF
    ...
    generateYawlServices(engineDecomposition, editorDecomposition); //MLF
  }

  //MLF
  private void generateYawlServices(YDecomposition engineDecomposition, Decomposition editorDecomposition) {
    if(engineDecomposition instanceof YAWLServiceGateway && editorDecomposition.getService() != null) {
      ((YAWLServiceGateway)engineDecomposition).setYawlService(new YAWLServiceReference(editorDecomposition.getService(), null));
    }
  }

  private void generateDecompositionInputParameters(YDecomposition engineDecomposition,
                                                    Decomposition editorDecomposition) {
    //MLF: alphabetically sort the input variable list
    List variables = editorDecomposition.getVariables().getInputVariables();
    Collections.sort(variables);

    for(int i = 0; i < variables.size(); i++) {
      DataVariable editorInputVariable =
        (DataVariable) variables.get(i);
      ...
      generateEngineParameter(
          ...
          i,
          editorInputVariable.isMandatory(),
          editorInputVariable.getAttributes()
      );
    }  
  }

  //MLF: This method appears to convert the editor vars to engine params
  private void generateEngineParameter(YDecomposition engineDecomposition,
    ...
    //MLF: transfer attributes to parameter
    for(Enumeration enumer = attributes.keys(); enumer.hasMoreElements();)
    {
        String key = enumer.nextElement().toString();
        engineParameter.addAttribute(key, XMLUtilities.quoteSpecialCharacters(attributes.get(key).toString()));
    }

    //MLF: provide mandatory & order
    engineParameter.setMandatory(mandatory);
    engineParameter.setOrdering(order);
  ...
  private void generateDecompositionOutputParameters(YDecomposition engineDecomposition, 
                                                    Decomposition editorDecomposition) {
    //MLF: alphabetically sort the input variable list
    List variables = editorDecomposition.getVariables().getOutputVariables();
    Collections.sort(variables);
    ...
    for(int i = 0; i < variables.size(); i++) {
      ...
      generateEngineParameter(
          ...
          i,
          editorOutputVariable.isMandatory(),
  ...
  private void setFlows(YNet engineNet, NetElementSummary editorNetSummary) {
    ...
    while(flowIterator.hasNext()) {
      ...
      //MLF: set the flow documentation, if present
      if (editorFlow.getUserObject() != null) {
        firstEngineFlow.setDocumentation(XMLUtilities.quoteSpecialCharacters(editorFlow.getUserObject().toString())); //MLF: user object is the label
      }
