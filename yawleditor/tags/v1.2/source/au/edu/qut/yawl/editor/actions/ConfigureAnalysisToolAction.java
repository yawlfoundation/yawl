...
class ConfigureAnalysisDialog extends AbstractDoneDialog {
  ...
  public ConfigureAnalysisDialog() {
    ...
    getDoneButton().addActionListener(new ActionListener(){
       public void actionPerformed(ActionEvent e) {
         prefs.putBoolean(
             "wofYawlRelaxedSoundnessCheck",
             relaxedSoundnessCheckBox.isSelected()
         );

         prefs.putBoolean(
             "wofYawlTransitionInvariantCheck",
             transitionInvariantCheckBox.isSelected()
         );

         prefs.putBoolean(
             "wofYawlExtendedCoverabilityCheck",
  ...
  private JPanel getConfigurationPanel() {
    ...
    panel.add(
      new JLabel(
          "<html>Both tests identify unreachable tasks in the specification,<br>" + 
          "potential deadlocks and possible unfinished work in completed<br>" +
          "workflow cases.</html>"
      ),
      gbc
    );
  ...
  private JCheckBox getRelaxedSoundnessCheckBox() {
    relaxedSoundnessCheckBox = new JCheckBox(
        "Check for relaxed soundness in a bounded analysis net"
    );
  ...
  private JCheckBox getTransitionInvariantCheckBox() {
    transitionInvariantCheckBox = new JCheckBox(
        "Check for semi-positive transition invariants in a short-circuited analysis net"
    );
  ...
  public void setVisible(boolean visible) {
    if (visible){
      relaxedSoundnessCheckBox.setSelected(
          prefs.getBoolean("wofYawlRelaxedSoundnessCheck", true)
      );
      
      transitionInvariantCheckBox.setSelected(
          prefs.getBoolean("wofYawlTransitionInvariantCheck", true)
      );

      extendedCoverabilityCheckBox.setSelected(
          prefs.getBoolean("wofYawlExtendedCoverabilityCheck", true)
      );
    }
