...
public class ELTYrule extends YAWLReductionRule{
    ...
    public YNet reduceElement(YNet net, YExternalNetElement netElement) {
        if (netElement instanceof YTask) {
            ...
            if (presetEqualsPostset(task) && hasOneInFlow(task) &&
                    !(canReset(task) || !isInACancelSet(task))) {

                YCondition condition = (YCondition) task.getPresetElements().iterator().next();

                // p is not in the cancellation region
                if (! isInACancelSet(condition)) {
