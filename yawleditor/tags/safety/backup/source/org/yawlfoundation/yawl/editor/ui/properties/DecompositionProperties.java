...
public class DecompositionProperties extends CellProperties {
    ...
    public String getCustomService() {
        String service = getWSDecomposition() != null ?
                getWSDecomposition().getServiceDescription() :
                ServicesPropertyEditor.DEFAULT_WORKLIST;

        // update for old specs
        if (service == null || service.equals("Default Engine Worklist")) {
    ...
    public void setCustomService(String service) {
        if (getWSDecomposition() != null) {
            getWSDecomposition().setService(ServicesPropertyEditor.getService(service));
            setDirty();
            enableServiceProperties(service);
        }
    }
    ...
    public void setAutomated(boolean auto) {
        if (getWSDecomposition() != null) {
            getWSDecomposition().setManualInteraction(! auto);
            setReadOnly("Codelet", ! auto);
            setDirty();
    ...
    public String getCodelet() {
        if (getWSDecomposition() != null) {
            String codelet = getWSDecomposition().getCodelet();
    ...
    public void setCodelet(String codelet) {
        if (getWSDecomposition() != null) {
            ...
            getWSDecomposition().setCodelet(codelet);
            setDirty();
        }
    }
    ...
    public NetTaskPair getTaskDataVariables() {
        return new NetTaskPair(specificationHandler.getRootNet(),
                _decomposition.getNet(), (YAWLTask) vertex);
    }
    ...
    private WebServiceDecomposition getWSDecomposition() {
        if (_decomposition instanceof WebServiceDecomposition) {
            return (WebServiceDecomposition) _decomposition;
        }
        return null;
    }
