...
public class YDataHandler {
     ...
     * @param type one of INPUT, OUTPUT or LOCAL
     * @throws YDataHandlerException if there is no specification associated with this
     * handler, or it has no decomposition with the specified id, or the 'type'
     * parameter is LOCAL and the referenced decomposition is not a net, or the 'type'
     * parameter is invalid
     */
    public void removeVariable(String decompositionID, String variableName, int type)
            throws YDataHandlerException {
        ...
        if (decomposition instanceof YNet) {
            ...
            if (type == LOCAL) raise("No net found with id: " + decompositionID);
            removeTaskDecompositionVariable(decomposition, variableName, type);
        }
    }
    ...
    public void renameVariable(String decompositionID, String oldName, String newName,
                               int type) throws YDataHandlerException {
        checkParameterType(type);
        YDecomposition decomposition = getDecomposition(decompositionID);
        if (decomposition instanceof YNet) {
     ...
     * 'type' parameter is invalid, or the 'index' parameter is negative
     */
    public boolean setVariableIndex(String decompositionID, String variableName,
                                       int type, int index) throws YDataHandlerException {
        checkParameterType(type);
     ...
     */
    public void setInitialValue(String netID, String variableName, String value)
            throws YDataHandlerException {
     ...
     */
    public void setDefaultValue(String decompositionID, String variableName, String value)
            throws YDataHandlerException {
        YDecomposition decomposition = getDecomposition(decompositionID);
     ...
     * @param type one of LOCAL, INPUT, OUTPUT or INPUT_OUTPUT (type INPUT_OUTPUT will
     ...
     *              to the variable. The value is ignored if the type is not LOCAL or
     *              OUTPUT only. May be null.
     * @throws YDataHandlerException if there is no specification associated with this
     * handler, or it has no net or task decomposition with the specified id, or the
     * 'type' parameter is invalid, or the net or task decomposition already has a
     * variable of that name
     */
    public void addVariable(String decompositionID, String name, String dataType,
                            String namespace, int type, String value)
            throws YDataHandlerException {
        checkParameterType(type);
        YDecomposition decomposition = getDecomposition(decompositionID);
        checkParameterNameUnique(decomposition, name, type);
        if (type == LOCAL) {
        ...
        if (type == INPUT || type == INPUT_OUTPUT) {
        ...
        if (type == OUTPUT || type == INPUT_OUTPUT) {
     ...
     * @param type one of LOCAL, INPUT, OUTPUT or INPUT_OUTPUT
     * @return true if successful, false if a variable of that name doesn't exist
     * @throws YDataHandlerException if there is no specification associated with this
     * handler, or it has no net or task decomposition with the specified id, or the
     * 'dataType' parameter is invalid, or the variable has a value that is incompatible
     * with the new data type, or the 'type' parameter is invalid
     */
    ...
    public boolean setVariableDataType(String decompositionID, String variableName,
                        String dataType, int type) throws YDataHandlerException {
        checkParameterType(type);
        YDecomposition decomposition = getDecomposition(decompositionID);
        ...
        if (type == LOCAL) {
            ...
            else raise("Invalid parameter type for task decomposition: " +
                    getScopeName(type));
        }
        else {
            if (type == INPUT || type == INPUT_OUTPUT) {
     ...
     * found with the specified id, or the 'type' parameter is invalid
     */
    public void setVariableMapping(String netID, String taskID,
                                   String variableName, String mapping, int type)
            throws YDataHandlerException {
        checkParameterType(type);
        YNet net = getNet(netID);
        ...
        if (task != null) {
            ...
            else raise("Invalid type parameter: " + type);
        }
    ...
    private void renameTaskDecompositionVariable(YDecomposition decomposition,
                                   String oldName, String newName, int type) {
        Set<YTask> affectedTasks = getTasks(decomposition);
        if (type == INPUT) {
            ...
            for (YTask task : affectedTasks) {
                ...
                if (query != null) {
                    task.setDataBindingForInputParam(renameTags(query, newName), newName);
                }
            }
        }
        else {
    ...
    private void removeReferencingTaskMappings(YNet net, String name) {
        for (YTask task : net.getNetTasks()) {
            ...
            removeReferencingFlowPredicates(task.getPresetFlows(), name);
    ...
    private void updateReferencingTaskMappings(YNet net, String oldName, String newName) {
        for (YTask task : net.getNetTasks()) {
            ...
            for (String outputQuery : removeKeys) {
                ...
                task.getDataMappingsForTaskCompletion().put(
                        renameTags(outputQuery, newName), newName);
            }

            // remove old keys
            removeTaskMappings(task.getDataMappingsForTaskCompletion(), removeKeys);

            amendReferencingFlowPredicates(task.getPresetFlows(), oldName, newName);
     ...
     */
    private String xpathDelimit(String label) {
        return '/' + label + '/';
    }
     ...
     */
    private String renameTags(String element, String newName) {
    ...
    /**
     * Checks that a type value is a LOCAL, INPUT or OUTPUT type
     * @throws YDataHandlerException if the type value is invalid
     */
    private void checkParameterType(int type) throws YDataHandlerException {
        switch (type) {
            case LOCAL:
            case INPUT:
            case OUTPUT:
            case INPUT_OUTPUT: break;
            default: raise("Invalid type parameter: " + type);
        }
    }


    private void checkParameterNameUnique(YDecomposition decomposition, String name,
                                          int type) throws YDataHandlerException {
        if (type == LOCAL) {
            if (decomposition instanceof YNet) {
