...
import org.yawlfoundation.yawl.editor.core.util.DataUtil;
...
public class YEditorSpecification {
    ...
    public List<String> getAllDataTypeNames() {
        return DataUtil.getDataTypeNames(getSchema());
    }


    public int getNetCount() {
        if (_specification == null) return 0;
        Set<YDecomposition> decompositions = _specification.getDecompositions();
        return decompositions != null ? decompositions.size() : 0;
    }


    public YNet createRootNet(String netName) {
        if (_specification == null) newSpecification();
    ...
    public YAWLServiceGateway addTaskDecomposition(String name) {
        ...
        // temp block - to remove when editor Decomposition is removed
        if (_specification.getDecomposition(name) != null) {
            return (YAWLServiceGateway) _specification.getDecomposition(name);
        }
    ...
    public void saveAs(String file, FileSaveOptions saveOptions, YLayout layout)
            throws IOException {
        setLayout(layout);
        _fileOps.saveAs(file, getMetaData(), saveOptions);
        resetModified();
