...
public class TaskPrivileges {
    ...
    private void allowAll(int privilege) {
        _allowAll[privilege] = true ;
    }

    private void disallowAll(int privilege) {
    ...
    private void add(HashSet<Participant> set, String uid) {
        Participant p = ResourceManager.getInstance().getParticipant(uid) ;
        add(set, p) ;
    }
    ...
    public void addRoleToPrivilegeUnchecked(String rid, int priv) {
        Role r = new Role() ;
        r.setID(rid);
//        getPrivSet(priv).add(r);
    }

    /** convenience methods */

    public boolean canSuspend(Participant p) {
        return allAllowed(CAN_SUSPEND) || contains(_canSuspend, p) ;
    }

    public boolean canReallocateStateless(Participant p) {
        return allAllowed(CAN_REALLOCATE_STATELESS) ||
               contains(_canReallocateStateless, p) ;
    }

    public boolean canReallocateStateful(Participant p) {
        return allAllowed(CAN_REALLOCATE_STATEFUL) ||
               contains(_canReallocateStateful, p) ;
    }

    public boolean canDeallocate(Participant p) {
        return allAllowed(CAN_DEALLOCATE) || contains(_canDeallocate, p) ;
    }

    public boolean canDelegate(Participant p) {
        return allAllowed(CAN_DELEGATE) || contains(_canDelegate, p) ;
    }

    public boolean canSkip(Participant p) {
        return allAllowed(CAN_SKIP) || contains(_canSkip, p) ;
    }

    public boolean canPile(Participant p) {
        return allAllowed(CAN_PILE) || contains(_canPile, p) ;
    }
    ...
    public void addSkipByRoleID(String roleID) {
    ...
    public void parse(Element e) {
        if (e != null) {
            ...
            while (itr.hasNext()) {
                ...
                String privName = ePrivilege.getChildText("name") ;
                HashSet<Participant> privSet = getPrivSet(privName) ;

                // are all allowed?
                String allowall = ePrivilege.getChildText("allowall");
                if (allowall != null) allowAll(getPriv(privName)) ;
    ...
    private String buildXMLForPrivilege(int priv) {
        ...
        String name = getPrivString(priv) ;
        if (allAllowed(priv) || (! pSet.isEmpty())) {
