...
public class InterfaceA_EnvironmentBasedClient extends Interface_Client {
    ...
    public Set getResourceConstraints(String sessionHandle) {
        Set constraints = new HashSet();

         try {
            String result = executeGet(_backEndURIStr +
                    "?action=getResourceConstraints&sessionHandle=" + sessionHandle);

            if (result != null && successful(result)) {
                Document doc = JDOMConversionTools.stringToDocument(result) ;

                Iterator itr = doc.getRootElement().getChildren().iterator();
                while (itr.hasNext()) {
                    Element elConstraint = (Element) itr.next();
                    constraints.add(ResourceConstraint.unmarshal(elConstraint));
                }
            }
        }
        catch (IOException e) {
    ...
    public Set getResourceFilters(String sessionHandle) {
        Set filters = new HashSet();

         try {
            String result = executeGet(_backEndURIStr +
                    "?action=getResourceFilters&sessionHandle=" + sessionHandle);
            ...
            if (result != null && successful(result)) {
                ...
                Iterator itr = doc.getRootElement().getChildren().iterator();
                while (itr.hasNext()) {
                    Element elFilter = (Element) itr.next();
                    filters.add(ResourceFilter.unmarshal(elFilter));
                }
            }
        }
        catch (IOException e) {
    ...
    public Set getResourceAllocators(String sessionHandle) {
        Set allocators = new HashSet();

         try {
            String result = executeGet(_backEndURIStr +
                    "?action=getResourceAllocators&sessionHandle=" + sessionHandle);
            ...
            if (result != null && successful(result)) {
                ...
                Iterator itr = doc.getRootElement().getChildren().iterator();
                while (itr.hasNext()) {
                    Element elAllocator = (Element) itr.next();
                    allocators.add(ResourceAllocator.unmarshal(elAllocator));
                }
            }
        }
        catch (IOException e) {
    ...
    public String getAllParticipantNames(String sessionHandle) {
         String result = null ;
         try {
            result = executeGet(_backEndURIStr +
                    "?action=getAllParticipantNames&sessionHandle=" + sessionHandle);
        }
        catch (IOException e) {
            InterfaceBWebsideController.logContactError(e, _backEndURIStr);
        }
    ...
    public String getAllRoleNames(String sessionHandle) {
         String result = null ;
         try {
            result = executeGet(_backEndURIStr +
                    "?action=getAllRoleNames&sessionHandle=" + sessionHandle);
        }
        catch (IOException e) {
            InterfaceBWebsideController.logContactError(e, _backEndURIStr);
        }
