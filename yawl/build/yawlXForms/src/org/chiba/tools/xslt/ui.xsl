...
<xsl:stylesheet version="1.0"
    ...
    <!-- ### wether JavaScript should be generated or not ### -->
            ...
            <tr>
                <xsl:for-each select="./*/xforms:label">
                    ...
                    <td id="{../@id}-label" class="{$label-class}">
                        <xsl:apply-templates select="self::node()[not(name(..)='xforms:trigger' or name(..)='xforms:submit')]" xmlns:xforms="http://www.w3.org/2002/xforms"/>
                    </td>
                </xsl:for-each>
            </tr>
            <tr>
