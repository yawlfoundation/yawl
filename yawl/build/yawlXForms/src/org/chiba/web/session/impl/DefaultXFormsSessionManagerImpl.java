 ...
 * @version $Id: DefaultXFormsSessionManagerImpl.java,v 1.2 2006/09/18 20:22:00 joernt Exp $
 *          <p/>
...
public class DefaultXFormsSessionManagerImpl extends Thread implements XFormsSessionManager {
    ...
    private static final int DEFAULT_TIMEOUT = 5000; //default is 5 Minutes
    ...
    public static DefaultXFormsSessionManagerImpl getInstance() {
        if (instance == null) {
            instance = new DefaultXFormsSessionManagerImpl();
        }
        return instance;
    ...
    /**
     * set the interval the wiper thread will check for expired sessions. Setting a value of -1 causes the wiper
     * thread to be *not* started.
     *
    ...
    private DefaultXFormsSessionManagerImpl() {
    ...
    public void wipe() {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Wiping expired sessions");
        }
    ...
    public synchronized void start() {
        if (this.interval != -1 && !threadStarted) {
            super.start();
