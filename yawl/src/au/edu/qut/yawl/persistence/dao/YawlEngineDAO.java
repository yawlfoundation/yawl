...
import au.edu.qut.yawl.persistence.DatasourceRoot;
...
public class YawlEngineDAO implements DAO {
	...
	private static final Log LOG = LogFactory.getLog( YawlEngineDAO.class );
...
//      String filter = "";
        ...
        if(parent instanceof DatasourceRoot) {
            try {
                retval.addAll((List) execute(new ListSpecificationsCommand()));
            }
            catch(Exception e) {
                throw new YPersistenceException("Error retrieving specification list from engine!", e);
            }
        }
...
//  }
	...
	public class ListSpecificationsCommand implements RemoteCommand {
	...
	public class SaveCommand implements RemoteCommand {
		...
		public Object execute() throws Exception {
				String msg = iaClient.unloadSpecification(object.getID(), sessionHandle);
				if(!Interface_Client.successful(msg) && msg != null && !msg.contains("no such specification with id"))
					LOG.warn(msg);
