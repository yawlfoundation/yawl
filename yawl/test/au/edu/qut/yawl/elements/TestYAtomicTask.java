...
/**
 * 
 * Author: Lachlan Aldred Date: 28/04/2003 Time: 11:12:39
 * 
 */
public class TestYAtomicTask extends AbstractTransactionalTestCase {
	 ...
	 */
	public TestYAtomicTask() {
		super();
	}

	public void setUp() throws Exception {
		super.setUp();
		engine = EngineFactory.createYEngine(true);
		...
		deadNet.initialise();
		_atomicTask1 = new YAtomicTask("AT1", YAtomicTask._AND,
				YAtomicTask._AND, deadNet);
		_atomicTask1.setDecompositionPrototype(new YAWLServiceGateway(
				_activityID, spec));
		_atomicTask1.setUpMultipleInstanceAttributes("3", "5", "3",
				YMultiInstanceAttributes._creationModeStatic);
		Map map = new HashMap();
		map.put("stub", "/data/stubList");
		_atomicTask1.setMultiInstanceInputDataMappings("stub",
				"for $d in /stubList/* return $d");
		...
		YAtomicTask before = new YAtomicTask("before", YAtomicTask._OR,
				YAtomicTask._AND, deadNet);
		YAtomicTask after = new YAtomicTask("after", YAtomicTask._OR,
				YAtomicTask._AND, deadNet);
		...
		_atomicTask2 = new YAtomicTask("at2", YAtomicTask._AND,
				YAtomicTask._AND, deadNet);
	...
	public void testFullAtomicTask() {
		...
		if (messages.size() > 0) {
			...
			System.out
					.println("(YVerificationMessage)messages.get(0)).class.getName( = "
							+ messages.get(0).getClass().getName());
	...
	public void testCancelTaskWhileRunning() throws YPersistenceException,
			YStateException, YDataStateException, YQueryException,
			YSchemaBuildingException {
		_atomicTask1.getParent().getParent().setBetaVersion(
				YSpecification._Beta7_1);
		...
		// for branch coverage, call cancel again (even though it's already
		// cancelled)
		_atomicTask1.cancel();
		assertFalse(_atomicTask1.t_isBusy());
	}
	...
	/**
	 * First version of this test cancels a task that's in the work item
	 * repository
	 */
	public void testCancelTaskInWorkItemRepositoryA()
			throws YPersistenceException, YStateException, YDataStateException,
...
//		 repos.clear();
	...
	/**
	 * Tests the cancellation of a task that's in the repository and that's tied
	 * to an actual YAWL service.
	 * 
	 * @throws Exception
	 */
	public void testCancelTaskInWorkItemRepositoryB() throws Exception {
		...
		spec = SpecReader.readSpecification(fileName, isXmlFileInPackage,
		...
		YAtomicTask task = (YAtomicTask) root.getInputCondition()
				.getPostsetElements().get(0);
...
//		repos.clear();
	...
	public void testEnablementMappingsSpecXML() throws Exception {
		...
		xml = "<specificationSet xmlns=\"http://www.yawl.fit.qut.edu.au/\" "
				...
				+ " d:/yawl/schema/YAWL_SchemaBeta7.1.xsd\">" + xml
				+ "</specificationSet>";

		YSpecification spec2 = (YSpecification) YMarshal
				.unmarshalSpecifications(xml, "testEnablementMappingsSpecXML")
				.get(0);

		// test some things to see if they're the same
		// (would be better if there was a .equals for YSpecification that
		// tested
		...
		assertTrue(spec.getDecompositions().size() == spec2.getDecompositions()
				.size());
		assertTrue(spec.getMetaData().toXML().equals(
				spec2.getMetaData().toXML()));
		assertTrue(spec.getRootNet().toXML().equals(spec2.getRootNet().toXML()));
		assertTrue(spec.toXML().equals(spec2.toXML()));
	}

	/**
	 * Tests having enablement mappings with a task that decomposes to null (ie:
	 * doesn't decompose to anything). Uses an XML file where the task does have
	 * a decomposition, then modifies it in the java code.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Tests enablement parameters that have an element name but no name.
	 * Compare the xml file lines 41 through 44 with lines 45 through 50. Note
	 * that this fails under the newer versions (when it checks the schema) but
	 * works with older YAWL versions.
	 * 
	 * @throws Exception
	 */
	public void testEnablementMappingsSpecC() throws Exception {
		...
		children = netRunner1.attemptToFireAtomicTask("SignOff_3");
		assertFalse("Spec input condition contains identifier", spec
				.getRootNet().getInputCondition().containsIdentifier());
		...
		assertTrue("Task 'SignOff_3' busy:" + netRunner1.getBusyTasks(),
				contains);
	}

	/**
	 * Checks that a specification with a syntax error causes the verify
	 * function to return verification errors, but not throw an exception. I
	 * don't remember exactly what the syntax error was, but I believe it is on
	 * line 38 of the xml:<br>
	 * <code>
	...
	public void testEnablementMappingsSpecD() {
		...
		try {
			spec = SpecReader.readSpecification(fileName, isXmlFileInPackage,
					TestYAtomicTask.class);
			fail("Spec " + fileName
					+ " was read successfully, but should have failed.");
	...
	/**
	 * Tests verification of a spec containing no root net, and where the
	 * enablement mappings of a task and the enablement parameters of the task's
	 * decomposition don't match up.
	 * 
	 * @throws Exception
	 */
	public void testEnablementMappingsSpecE() throws Exception {
		...
		assertTrue("Spec validation should return an error message",
				errorMsgExists);
	}
	...
	public void testEnablementMappingsSpecXML_F() throws Exception {
		...
		YSpecification spec2 = (YSpecification) YMarshal
				.unmarshalSpecifications(xml, "testEnablementMappingsSpecF_XML")
				.get(0);
	...
	/**
	 * Tests that an atomic task that does not have a decomposition causes the
	 * verify function to return verification errors, but not throw any
	 * exceptions.
	 * 
	 * @throws Exception
	 */
	public void testTaskWithoutDecompositionSpec() throws Exception {
		...
		List<YVerificationMessage> messages = readAndVerifySpec(fileName,
				isXmlFileInPackage);
		...
		assertTrue(fileName + " verified OK, should have gotten an error",
				containsError);
	}

	/**
	 * Tests that an atomic task that decomposes to a Net causes the verify
	...
	public void testTaskDecomposesToNetSpec() throws Exception {
		...
		if (root == null || otherNet == null) {
			fail(fileName
					+ " should have nets called 'OverseeMusic' and 'leaf-c'"
					+ root + " " + otherNet);
		}
	 ...
	 * @throws Exception
	 */
	private List<YVerificationMessage> readAndVerifySpec(String fileName,
			boolean isXmlFileInPackage) throws Exception {
		List<YVerificationMessage> messages = new LinkedList<YVerificationMessage>();

		YSpecification spec = SpecReader.readSpecification(fileName,
				isXmlFileInPackage, TestYAtomicTask.class);
		messages = verifySpec(spec, fileName);
	...
	/**
	 * Helper function that attempts to verify the given specification and
	 * return the verification messages.
	 */
	private List<YVerificationMessage> verifySpec(YSpecification spec,
	...
	public static void main(String[] args) {
		try {
			TestYAtomicTask test = new TestYAtomicTask();
			test.setUp();
