...
<%@ page import="org.yawlfoundation.sb.revisecastinfo.*"%>
	...
	if (MultipartFormDataRequest.isMultipartFormData(request)) 
	{
		...
		System.out.println("mrequest jsessionid: "+request.getParameter("JSESSIONID"));
	     ...
	     if ( (todo != null) && (todo.equalsIgnoreCase("upload")) )
	     {
            ...
            int endOfFile = result.indexOf("</ns2:Revise_Cast_List>");
		...
		xml = xml.replaceAll("<Revise_Cast_List", "<ns2:Revise_Cast_List xmlns:ns2='http://www.yawlfoundation.org/sb/reviseCastInfo'");
		xml = xml.replaceAll("</Revise_Cast_List","</ns2:Revise_Cast_List");
		//System.out.println("outputData xml: "+xml+" --- ");
	}
	
	ByteArrayInputStream xmlBA = new ByteArrayInputStream(xml.getBytes());
	JAXBContext jc = JAXBContext.newInstance("org.yawlfoundation.sb.revisecastinfo");
	Unmarshaller u = jc.createUnmarshaller();
	JAXBElement rclElement = (JAXBElement) u.unmarshal(xmlBA);	//creates the root element from XML file	            
	ReviseCastListType rcl = (ReviseCastListType) rclElement.getValue();
...
<table width="700" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<form name="form1" method="post">
              ...
              <td><table width="700" border="0" cellpadding="0" cellspacing="0" id="crew">
				<tbody>
                  ...
                  <tr>
					...
					<%int a=0;
					if (rcl.getCastInfo() != null) {
					CastInfoType cit = rcl.getCastInfo();
					if (cit.getSingleCastInfo().isEmpty() == false){
		...
		<!-- LOAD -->
    <form method="post" action="Revise_Cast_List_7864.jsp?formType=load&workItemID=<%= request.getParameter("workItemID") %>&userID=<%= request.getParameter("userID") %>&sessionHandle=<%= request.getParameter("sessionHandle") %>&JSESSIONID=<%= request.getParameter("JSESSIONID") %>&submit=htmlForm" name="upform" enctype="MULTIPART/FORM-DATA">
...
if(request.getParameter("Submission") != null){
	...
	File f = new File("./backup/ReviseCastList_"+new SimpleDateFormat("dd-MM-yyyy_hh-mm-ss").format(new Date())+".xml");
	...
	response.setHeader("Content-Disposition", "attachment;filename=\"ReviseCastList_"+new SimpleDateFormat("dd-MM-yyyy_hh-mm-ss").format(new Date())+"_l.xml\";");
